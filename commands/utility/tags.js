...
class PingCommand extends Subcommand {
  constructor(context, options) {
    super(context, {
      ...
      detailedDescription: {
        ...
        examples: ['tag list', 'tag breezip', 'youtube add sylveon I like Sylveons'],
        args: ['subcommand: Can be list/add/remove/lock/display. Defaults to video.', 'name: The name of the tag.']
      },
      subcommands: [
        {
          name: 'list',
  ...
  registerApplicationCommands(registry) {
    ...
    registry.registerChatInputCommand((builder) =>
      ...
      .addStringOption(option => option.setName('name').setDescription('The name of the tag').setRequired(true)))
      .addSubcommand((command) => command.setName('lock').setDescription('Locks tag management to server managers.')
      .addBooleanOption(option => option.setName('enable').setDescription('Enable the option?').setRequired(true)))
  ...
  async chatInputLock(interaction) {
    ...
    db.save()
    .then(() => { interaction.followUp(`:white_check_mark: ${option ? 'locked tags. You now need to be a server manager to manage tags.' : 'unlocked tags.'}`); })
  ...
  async chatInputList(interaction) {
    ...
    interaction.followUp(`The following indexes are available.${db.lockTags ? ' Tags in this server can only be managed by a server manager.' : ''}\n\n**Built-in:** ${indexes.map(i => `\`${i.name}\``)}\n**Server tags:** ${srvTags.length > 0 ? srvTags.map(r => `\`${r.name}\``) : 'No tags created.'}`);
  }
  ...
  async messageLock(message, args) {
    ...
    db.save()
    .then(() => { message.reply(`:white_check_mark: ${option ? 'locked tags. You now need to be a server manager to manage tags.' : 'unlocked tags.'}`); })
  ...
  async messageAdd(message, args) {
    ...
    if (tagDesc.length > 256) return message.reply(':x: Tag description is too long.');
    if (db.tags.length > 25) return message.reply(':x: You\'ve maxed out on the maximum of tags you can hold in the server. Limit is 25.');
    ...
    db.save()
    .then(() => { message.reply(`:white_check_mark: Successfully added tag \`${tagName}\`.`); })
    .catch((err) => { message.reply(`:x: ${err}`); });
  }
  ...
  async messageRemove(message, args) {
    ...
    db.save()
    .then(() => { message.reply(`:white_check_mark: Successfully removed tag \`${tagName}\`.`); })
    .catch((err) => { message.reply(`:x: ${err}`); });
  }
  ...
  async messageList(message) {
    ...
    message.reply(`The following indexes are available.${db.lockTags ? ' Tags in this server can only be managed by a server manager.' : ''}\n\n**Built-in:** ${indexes.map(i => `\`${i.name}\``)}\n**Server tags:** ${srvTags.length > 0 ? srvTags.map(r => `\`${r.name}\``) : 'No tags created.'}`);
  }
