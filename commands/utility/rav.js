...
class PingCommand extends Command {
  constructor(context, options) {
    super(context, {
      ...
      detailedDescription: {
        ...
        flags: [
          `--links : Provides links to search the user's avatar instead.`,
        ],
      },
      cooldownDelay: 3_000,
  ...
  async messageRun(message, args) {
    ...
    .then((data) => {
        if (!data.image_results) return message.reply(`:x: No similar profile pictures found.`);
        const othermatches = data.image_results.map(d => `[[${d.position}]](<${d.link}>)`);
        message.reply(`Rav has found ${othermatches.length} possible matches for ${user.username}. They will be listed below. Alternative you can use the --links flag to send links to search it instead.\n**First match:** ${data.image_results[0].link}\n**All matches:** ${othermatches}`);
    })
