...
const { EmbedBuilder, Colors, ActionRowBuilder, ButtonBuilder, ButtonStyle, PermissionFlagsBits } = require('discord.js');
...
class PingCommand extends Subcommand {
    ...
    super(context, {
      ...
      aliases: ['verify'],
        ...
        detailedDescription: {
          ...
          examples: ['verification display', 'verification set @verified "Click below to verified" "Welcome to the server, {{mention}}', 'verification clear'],
  ...
  async messageDisplay(message) {
    ...
    const embed = new EmbedBuilder()
    .setAuthor({ name: message.guild.name + ' Verification', iconURL: message.guild.iconURL({ dynamic: true }) })
    .setDescription(db.verification.messageText ? await require('../../tools/textParser').parse(db.verification.messageText, message.member) : 'This server uses PhoenixBot for verification. To verify, click the verification button below.')
    .setTimestamp(new Date())
    .setColor(Colors.Orange);
  ...
  async messageSet(message, args) {
    ...
    db.save()
    .then(() => { message.reply(`:white_check_mark: Successfully setup verification.`); })
  ...
  async messageClear(message) {
    ...
    db.save()
    .then(() => { message.reply(`:white_check_mark: Successfully cleared verification settings.`); })
    .catch((err) => { message.reply(`:x: ${err}`); });
  }
}
module.exports = {
