  ...
  async messageRun(message, args) {
    ...
    switch (option) {
        ...
        case 'test': {
            try {
              const serverdb = await database.findById(message.guild.id).exec();
              if (serverdb.welcomer.welcomertext == "") return send(message, `${emojis.error} Welcomer isn't setup. Run \`welcomer setup\` to setup.`);
        ...
        default: {
            ...
            if (verif.channel == "") return send(message, `${emojis.error} Welcomer isn't setup. Run \`welcomer setup\` to setup.`);
