...
class PingCommand extends Subcommand {
  constructor(context, options) {
    super(context, {
      ...
      detailedDescription: {
        ...
        examples: ['goodbyes 1199497550143701042 {{username}} has left the server'],
  ...
  async messageDisplay(message) {
    ...
    if (!db.goodbyes.channel) return message.reply(`:x: Goodbyes is not setup.`);

    message.reply(`Goodbye messages are being sent to <#${db.goodbyes.channel}>\nMessage: ${await require('../../tools/textParser').parse(db.goodbyes.message, message.member)}`);
  }
  ...
  async messageSet(message, args) {
    ...
    db.save()
    .then(() => { message.reply(`:white_check_mark: Successfully setup goodbye.`); })
  ...
  async messageClear(message) {
    ...
    db.save()
    .then(() => { message.reply(`:white_check_mark: Successfully cleared goodbye settings.`); })
    .catch((err) => { message.reply(`:x: ${err}`); });
  }
}
module.exports = {
