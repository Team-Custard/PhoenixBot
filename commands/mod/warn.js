...
class PingCommand extends Command {
  ...
  async chatInputRun(interaction) {
    ...
    const silentDM = await interaction.options.getBoolean("silent");
    ...
    if (interaction.guild.members.me == member) {
    ...
    if (!silentDM) {
      const embeds = [
        new EmbedBuilder()
          ...
          .setDescription(
            `You were issued a warning in **${interaction.guild.name}**. You now have ${db.infractions.filter(i => (i.punishment == "Warn" && i.member == member.id)).length+1} warnings.\n**Case: \` ${thecase.id} \`**\n**Moderator:** ${hideMod ? "Hidden" : `<@!${thecase.moderator}>`}\n**Reason:** ${thecase.reason || "No reason was provided"}`,
          )
          .setFooter({
    ...
    if (system) {
      ...
      if (system.punishment == "kick") {
        ...
        member.kick(`(Warn by ${interaction.user.tag}) [${system.warnings} warnings] ${reason}`)
        .catch(function (err) {
          console.log(err);
          systemfailed = true;
        })
      ...
      if (system.punishment == "shadowban") {
        ...
        const newRoles = [db.moderation.shadowBannedRole].concat(member.roles.cache.filter(r => !r.editable || r.managed))
        ...
        await member.roles.set(newRoles, `(Warn by ${interaction.user.tag}) [${system.warnings} warnings] ${reason}`)
        .catch(function (err) {
          console.log(err);
          systemfailed = true;
        })
      ...
      if (system.punishment == "ban") {
        ...
        member.ban({ reason: `(Warn by ${interaction.user.tag}) [${system.warnings} warnings] ${reason}` })
        .catch(function (err) {
          console.log(err);
          systemfailed = true;
        })
        ...
        if (sysduration > 0 && !systemfailed) {
          this.container.tasks.create({ name: 'tempBan', payload: { guildid: interaction.guild.id, memberid: member.id, caseid: thecase2.id } }, { delay: sysduration, customJobOptions: { removeOnComplete: true, removeOnFail: true } });
        }
      }
    }
    ...
    if (db.logging.infractions) {
      ...
      if (channel) {
        const embedT = new EmbedBuilder()
          ...
          .setDescription(
            `**Offender:** ${member}\n**Moderator:** ${hideMod ? "Hidden" : interaction.user}\n**Reason:** ${thecase.reason}`,
          )
          .setColor(Colors.Orange)
