...
class PingCommand extends Command {
  constructor(context, options) {
    super(context, {
      ...
      name: "reason",
      aliases: [`setreason`],
      description: "Changes the reason of an infraction. Note changing infractions makes you the responsible moderator.",
      detailedDescription: {
        usage: "reason <caseid> <newreason>",
        examples: ["reason 22 Being mean", "reason 4 Insults"],
        args: ["caseid : The case to modify", "newreason : The new reason for the infraction"]
      },
      cooldownDelay: 3_000,
  ...
  async messageRun(message, args) {
    ...
    await db.save();
    message.reply(`:white_check_mark: Modified reason for case  **\` ${caseid} \`**.`);
    ...
    if (db.logging.infractions) {
        ...
        if (channel) {
          ...
          await channel
            .messages.fetch(thecase.modlogID).then(function (msg) {
