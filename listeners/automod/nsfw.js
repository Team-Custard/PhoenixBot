...
const { EmbedBuilder, AttachmentBuilder, Colors, Permissions } = require("discord.js");
...
class ReadyListener extends Listener {
  ...
  async run(message) {
    ...
    if (db.automod.nsfwimage.length > 0) {
        if (!model) model = await nsfwjs.load(`file://./tools/automod/nsfwmodel.json`);

        let wasflagged = false;
        const images = message.attachments.filter(a => a.contentType.startsWith('image')).map(a => a.url);
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            if (!imgfetch) { console.log(`Error fetching an image ${images[i]}`); }
            else {
                ...
                if (img) {
                    ...
                    switch (predictions[0].className.toLowerCase()) {
                        ...
                        case 'sexy': {
                            ...
                            console.log(`Dirty! it's ${predictions[0].className} (${(predictions[0].probability * 100).toString().substring(0, 3)}% certain) :) Time to ban ban ban!`);
                            break;
                        }
                        default: {
                            console.log(`Clean. it's ${predictions[0].className} (${(predictions[0].probability * 100).toString().substring(0, 3)}% certain)`);
                        }
                    }
                }
            }
        ...
        if (wasflagged == true) {
            ...
            if (executions.includes('report')) {
                ...
                .setDescription(`Message by ${message.author} was flagged by automod in ${message.channel}.`)
                .setColor(Colors.Orange)
                .addFields([
                    { name: "Content", value: (message.content ? message.content : `(None)`) },
                    { name: "Reason", value: `Detected an nsfw image ${flagReason}. Flagged images were attached to this message.` },
                ]);

                const reportChannel = await message.guild.channels.fetch(db.automod.reportchannel)
                .catch(() => undefined);
                if (reportChannel) {
                    const attach = [];
                    ...
                    reportChannel.send({
                        content: (db.automod.pingreport ? `<@&${db.automod.pingreport}>` : ""),
                        embeds: [embed],
                        files: attach
                    });
                }
            ...
            if (executions.includes('ban')) {
                if (!message.member.bannable) return;
                    ...
                    moderator: this.container.client.user.tag,
                ...
                .addFields(
                    ...
                    { name: `Reason:`, value: `(Automod) Nsfw image detected (${flagReason})` },
                )
                .setColor(Colors.Orange)
                ...
                await message.member.ban({ deleteMessageSeconds: 0, reason: thecase.reason });
                db.infractions.push(thecase);
                await db.save().catch(() => undefined);
                if (sendMessage == true) return message.channel(`:white_check_mark: ${message.author} was **banned** with case id **\` ${caseid} \`**.\n**Reason:** (Automod) Nsfw image detected (${flagReason}).`);
            }
                ...
                db.infractions.push(thecase);
                await db.save().catch(() => undefined);
                if (sendMessage == true) return message.channel.send(`:white_check_mark: ${message.author} was **kicked** with case id **\` ${caseid} \`**.\n**Reason:** (Automod) Nsfw image detected (${flagReason}).`);
            }
            else if (executions.includes('mute')) {
                if (!message.member.moderatable) return;
                let caseid = 0;
                const duration = require("ms")((db.automod.muteduration ? db.automod.muteduration : '20m'), { long: true });
                caseid = db.infractions.length + 1;
                const thecase = {
                    id: caseid,
                    punishment: "Mute",
                ...
                await message.member.disableCommunicationUntil(Date.now() + (duration), thecase.reason);
                db.infractions.push(thecase);
                await db.save().catch(() => undefined);
                if (sendMessage == true) return message.channel.send(`:white_check_mark: ${message.author} was **muted** for **${require("ms")(duration, { long: true })}** with case id **\` ${caseid} \`**.\n**Reason:** (Automod) Nsfw image detected (${flagReason}).`);
            }
                ...
                db.infractions.push(thecase);
                await db.save().catch(() => undefined);
                if (sendMessage == true) return message.channel.send(`:white_check_mark: ${message.author} was **warned** with case id **\` ${caseid} \`**.\n**Reason:** (Automod) Nsfw image detected (${flagReason}).`);
            }
            else if (sendMessage == true) return message.channel.send(`:warning: ${message.author} Don't send nsfw images.`);
        }
    }
  }
}
