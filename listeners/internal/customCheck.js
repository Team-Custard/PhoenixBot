  ...
  run(client) {
    setInterval(async function () {
        ...
        if (!foundToken) {
            ...
            const user = await client.users.fetch(former.user).catch(() => undefined);
            if (user) {
                const embed = new EmbedBuilder()
                .setTitle(`Your bot has expired`)
                .setDescription(`Hello ${user}, this is an update about your bot, **${client.user.username}**. It appears that your custom bot has expired, so it won't function anymore.`)
                .addFields([
                    {name: `What now?`, value: `> PhoenixBot will take over for the custom bot. Everything will continue to function as planned. If you don't have PhoenixBot, you can invite it [here](https://phoenix.sylveondev.xyz/invite).\n> Phoenix's prefix will set to the custom bot's prefix.\nThank you for your interest in using Custom Phoenix. We hope you had fun with our service.`},
                ])
