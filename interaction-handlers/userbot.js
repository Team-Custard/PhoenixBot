...
class MenuHandler extends InteractionHandler {
    ...
    parse(interaction) {
      if (interaction.customId == 'userbotinfo' || interaction.customId.startsWith("userbotephemeral")) return this.some();
    ...
    async run(interaction) {
        if (interaction.customId.startsWith("userbotephemeral")) {
          if (!interaction.customId.endsWith(interaction.user.id)) return interaction.reply({ content: `Sorry, you can't use that button on messages you don't invoke.`, ephemeral: true });
          await interaction.deferReply({ ephemeral: true });
          const db = await UserDB.findById(interaction.user.id);
          if (!db) return interaction.followUp(`:x: You need to configure UserDB before you can do that.`);
          db.ephemeral = (db.ephemeral ? false : true);
          db.save()
          .then(() => { interaction.followUp(`:white_check_mark: ${db.ephemeral ? `Enabled ephemeral output.` : `Disabled ephemeral output.`}`); })
          .catch((err) => { interaction.followUp(`:x: ${err}`); });
        }

        if (interaction.customId == "userbotinfo") {
            return interaction.reply({ content: `### About Userapps\nUserapps are a new feature by Discord that allows you to use supported bots in direct messages and servers that have not have the bot. ` +
